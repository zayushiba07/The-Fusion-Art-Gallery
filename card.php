   ...
   $user_id=$_SESSION['user_id'];
    ...
    if(isset($_POST['sub']))
    {
        ...
        mysqli_query($conn, "INSERT INTO `orders`(user_id, name, number, email, address, product_name, total_price, placed_on) VALUES('$user_id', '$name', '$number', '$email',  '$address', '$total_products', '$t_price', '$placed_on')") or die('query failed');
         mysqli_query($conn,"INSERT INTO `payment_status`( `user_id` , `name`, `product_name`, `price`, `payment_status`, `method`) VALUES ('$user_id','$name','$total_products','$t_price','padding','Credit Card')");
...
<div class="add-products">
    <form method="post">
     ...
     <input type="email" name="cname" class="box" placeholder="Enter your Email" required><br><br>
